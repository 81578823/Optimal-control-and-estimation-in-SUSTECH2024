...
class Actor(nn.Module):
    ...
    def forward(self, obs):
        ...
        # multiply by maximum action (3N) in order to scale the action appropriately
...
class DDPGAgent:
    def __init__(self, env, gamma, tau, buffer_maxlen, critic_learning_rate, actor_learning_rate, train, decay,device=torch.device('cpu')):    # change device to cuda when training
        #self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")   # use when training
        self.device = device
        ...
        # the exploration noise only be used during training
        if self.train == True:
            self.noise = OUNoise(self.env.action_space, decay_period=self.decay)

    def get_action(self, obs, t=0):
        #state = torch.FloatTensor(obs).unsqueeze(0).to(self.device)   # use when training
        state = torch.FloatTensor(obs).unsqueeze(0).to(device = 'cpu')
